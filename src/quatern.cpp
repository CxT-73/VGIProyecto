...
//		since the angle is assumed to be in radians
	double angle = double((degree / 180.0f) * glm::pi<float>() );
...
//		*x = tx * (1.0f / len);
		...
		degree= GLdouble((dgr_rad * 180.0f) / glm::pi<float>()); // Angle en graus
...
//make sure we do not divide by 0
	...
	else length = (GLdouble)glm::pi<float>()/2;
...
void QuatLnDif(GL_Quat q1, GL_Quat q2, GL_Quat &res)
{
	...
	s = QuatDot (q1, q2);
	if (s != 0.0) len1 = atan (len / s); else len1 = glm::pi<float>()/2;
	if (len != 0.0) len1 /= len;
	res.w = 0.0;
	res.x = dif.x * len1;
	res.y = dif.y * len1;
